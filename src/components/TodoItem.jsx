...
const TodoItem = ({
	...
	const updateTodo = async (id, completed) => {
		...
		try {
			...
			// 요청 본문에는수정된 TodoItem의 완료 상태를 포험
			await axios.put(`http://localhost:8080/api/todos/${id}`, CompleteTodo);
	...
	const saveEdit = async id => {
		// 4.4 TodoItem 제목 변경
		try {
			// 요청 본문에 담을 수정된 TodoItem의 제목과 완료 상태
			const EditTodo = {
				title: newTitle,
				completed: todo.completed,
			};

			// axios를 사용하여 서버의 /api/todos/:id 엔드포인트에 PUT 요청
			// 요청 본문에는 수정된 TodoItem의 제목과 완료 상태를 포함
			await axios.put(`http://localhost:8080/api/todos/${id}`, EditTodo);
	...
	const deleteTodo = async id => {
		// 4.5 TodoItem 삭제
		try {
			// axios를 사용하여 서버의 /api/todos/:id 엔드포인트에 DELETE 요청
